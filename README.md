...
<li>Вывести список сотрудников, чья зарплата превышает среднюю по вузу.</li>
	...
	```
	SELECT * FROM cypher('университет_граф', $$
	// Простой расчет: находим среднюю зарплату
	MATCH (s:Сотрудник)-[rel:Имеет_должность]->(:Должность)
	WITH AVG(rel.ставка) * COUNT(DISTINCT rel) / COUNT(DISTINCT s) AS примерная_средняя
	
	MATCH (s:Сотрудник)-[rel:Имеет_должность]->(:Должность)
	WITH s, SUM(rel.ставка) AS зарплата, примерная_средняя
	WHERE зарплата > примерная_средняя
	RETURN s.фио, зарплата, примерная_средняя
	ORDER BY зарплата DESC
	$$) AS t(фио agtype, зарплата agtype, примерная_средняя agtype);
	```
<li>Вывести список должностей с количеством ставок, на которые в каких-либо отлелах/кафедрах есть свободные вакансии.</li>
